...
\Title{2-Hilbert spaces give \texorpdfstring{$H^*$}{Hstar}-algebras}
...
% \eval\big(\bar{\psi}\,\big)=\overline{\eval(\psi)}$ for all $\psi\in \A(S^n)$, and (**P**) the induced sesquilinear pairing $\braket{a|b}_{c}\coloneqq \eval(\overline{a}\bullet_{c}b)$ is positive-definite for all string diagrams $c$ on $S^{n-1}$.
