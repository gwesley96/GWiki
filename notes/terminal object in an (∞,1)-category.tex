...
\begin{itemize}
\item The condition for $n=1$ says that for every object $c$ in $C$, there is a 1-cell $f\colon c\to y$.
\item The condition for $n=2$ implies that for any two parallel maps $f, g \colon c \to y$ in $C$, we have $[f]=[g]$, i.e., $f$ and $g$ are homotopic. Thus there is at most one homotopy class of maps into $y$ from any object.
\end{itemize}
...
See also:
\begin{itemize}
\item \wref{initial object in an (∞,1)-category} $C$ (just a terminal object of $C^{\mathrm{op}}$)
\item \wref[31.6. Uniqueness of initial and terminal objects]{QuasicategoriesRezk.pdf#page=79&selection=99,0,101,42&color=yellow}
\end{itemize}
