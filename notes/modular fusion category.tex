...
\begin{itemize}
\item $\mathcal{C}$ is \textbf{nondegenerate}, i.e., the \wref{S-matrix} of $\mathcal{C}$ is invertible.
\item The \wref{symmetric center} $\mathcal{C}'$ of $\mathcal{C}$ is trivial ($\mathcal{C}'\cong\mathsf{Vec}$).
\item $\mathcal{C}$ is \textbf{factorizable}, i.e., the \wref{braided monoidal functor} $\mathcal{C}\boxtimes \mathcal{C}^{\mathrm{rev}}\to\mathcal{Z}(\mathcal{C})$ given by the Deligne product of the functors $c\mapsto (c,\beta_{c,-})$ and $c\mapsto (c,\beta_{-,c})$ is an equivalence (where $\mathrm{rev}$ denotes $\mathcal{C}$ with braiding $\beta_{x,y}^{\mathrm{rev}}\coloneqq \beta_{y,x}^{^{-1}}$).
\end{itemize}
\textbf{\wref[Proposition 8.20.12]{EGNO15.pdf#page=260&selection=132,0,134,7}.}

\textbf{\wref[Theorem]{ColDelThesisQuantumSymmetries.pdf#page=18&selection=51,0,51,7} (\href{https://arxiv.org/abs/1509.06811}{here}).} A modular tensor category is equivalent to a 3-2-1 \wref[TQFT]{TQFT}.
...
\begin{itemize}
\item \wref{anyon}
\item \wref[Fibonacci topological order]{Fibonacci category}
\item \wref{Bulk–Boundary Correspondence}
\end{itemize}
